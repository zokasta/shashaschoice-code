    ...
    */
    protected function get_reader( $file ) {
        ...
        if ( ! class_exists( 'XMLReader' ) ) {
            //$this->logger->critical( esc_html__( 'The XMLReader class is missing! Please install the XMLReader PHP extension on your server', 'goldsmith' ) );
        ...
        if ( ! $status ) {
            //$this->logger->error( esc_html__( 'Could not open the XML file for parsing!', 'goldsmith' ) );
    ...
    */
    public function import( $file, $options = array() ) {
        ...
        if ( is_wp_error( $result ) ) {
            //$this->logger->error( esc_html__( 'Content import start error: ', 'goldsmith' ) . $result->get_error_message() );
        ...
        while ( $reader->read() ) {
            ...
            switch ( $reader->name ) {
                ...
                if ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) ) {
                    ...
                    __( 'This WXR file (version %s) is newer than the importer (version %s) and may not be supported. Please consider updating.', 'goldsmith' ),
                    $this->version,
    ...
    */
    public function new_ajax_request_maybe( $data ) {
        ...
        if ( $time > apply_filters( 'pt-importer/time_for_one_ajax_call', 20 ) ) {
            ...
            //$this->logger->info( esc_html__( 'New AJAX call!', 'goldsmith' ) );
        ...
        if ( strstr( $data['taxonomy'], 'pa_' ) ) {
            if ( ! taxonomy_exists( $data['taxonomy'] ) ) {
                ...
                if ( ! in_array( $attribute_name, wc_get_attribute_taxonomies() ) ) {
                    ...
                    if ( 'color' == $attribute_name || 'colors' == $attribute_name ) {
                        $type = 'color';
                    }
                    $attribute = array(
