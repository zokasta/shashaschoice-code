...
if ( ! function_exists( 'goldsmith_breadcrumbs' ) ) {
    ...
    function goldsmith_breadcrumbs( $args = array() ) {

        $breadcrumb = apply_filters( 'breadcrumb_trail_object', null, $args );

        if ( '0' == goldsmith_settings( 'breadcrumbs_visibility', '1' ) )
        return;

        if ( class_exists( 'WooCommerce' ) && is_woocommerce() && 'woo' == goldsmith_settings( 'shop_breadcrumbs_type', 'theme' ) ) {
            return woocommerce_breadcrumb();
        }

        if ( ! is_object( $breadcrumb ) )
        $breadcrumb = new Goldsmith_Breadcrumb( $args );
...
class Goldsmith_Breadcrumb {
    ...
    */
    public function trail() {
        ...
        if ( 0 < $item_count ) {
            ...
            $breadcrumb .= sprintf(
                '<%s class="trail-items goldsmith-breadcrumb">',
            ...
            $breadcrumb = sprintf(
                '<%1$s aria-label="%2$s" class="breadcrumb-trail goldsmith-breadcrumbs">%3$s%4$s%5$s</%1$s>',
    ...
    */
    protected function set_labels() {
        ...
        $defaults = array(
            'browse'              => esc_html__( 'Browse:',                               'goldsmith' ),
            'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'goldsmith' ),
            'home'                => esc_html__( 'Home',                                  'goldsmith' ),
            'error_404'           => esc_html__( '404 Not Found',                         'goldsmith' ),
            'archives'            => esc_html__( 'Archives',                              'goldsmith' ),
            // Translators: %s is the search query.
            'search'              => esc_html__( 'Search results for: %s',                'goldsmith' ),
            // Translators: %s is the page number.
            'paged'               => esc_html__( 'Page %s',                               'goldsmith' ),
            // Translators: %s is the page number.
            'paged_comments'      => esc_html__( 'Comment Page %s',                       'goldsmith' ),
            // Translators: Minute archive title. %s is the minute time format.
            'archive_minute'      => esc_html__( 'Minute %s',                             'goldsmith' ),
            // Translators: Weekly archive title. %s is the week date format.
            'archive_week'        => esc_html__( 'Week %s',                               'goldsmith' ),
    ...
    */
    protected function add_minute_hour_archive_items() {
        ...
        $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'goldsmith' ) ) );
    }
    ...
    */
    protected function add_minute_archive_items() {
        ...
        // Add the minute item.
        if ( true === $this->args['show_title'] )
        $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'goldsmith' ) ) );
    }
    ...
    */
    protected function add_hour_archive_items() {
        ...
        // Add the hour item.
        if ( true === $this->args['show_title'] )
        $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'goldsmith' ) ) );
    }
    ...
    */
    protected function add_day_archive_items() {
        ...
        $year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'goldsmith' ) ) );
        $month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'goldsmith' ) ) );
        $day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'goldsmith' ) ) );
    ...
    */
    protected function add_week_archive_items() {
        ...
        $year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'goldsmith' ) ) );
        $week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'goldsmith' ) ) );
    ...
    protected function map_rewrite_tags( $post_id, $path ) {
        ...
        if ( is_array( $matches ) ) {
            ...
            foreach ( $matches as $match ) {
                ...
                if ( '%year%' == $tag )
                $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'goldsmith' ) ) ) );

                // If using the %monthnum% tag, add a link to the monthly archive.
                elseif ( '%monthnum%' == $tag && true === $this->args['show_month'] )
                $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'goldsmith' ) ) ) );

                // If using the %day% tag, add a link to the daily archive.
                elseif ( '%day%' == $tag && true === $this->args['show_day'] )
                $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'goldsmith' ) ) ) );
