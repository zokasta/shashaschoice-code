...
class Hostinger_Ai_Assistant_Amplitude {
    ...
    public function ai_content_created( string $post_type, string $location = 'ai_assistant_ui', array $additional_properties = [] ): void {
        ...
        $this->amplitudeManager->sendRequest( $endpoint, $params, $headers );
    }

    public function ai_content_saved( string $post_type, int $post_id, string $location = 'ai_assistant_ui', array $additional_properties = [] ): void {
        $endpoint = self::AMPLITUDE_ENDPOINT;
        $params   = array(
    ...
    public function ai_content_published( string $post_type, int $post_id, string $location = 'ai_assistant_ui', array $additional_properties = [] ): void {
        $endpoint = self::AMPLITUDE_ENDPOINT;
        $params   = array(
	...
	public function track_published_post( string $new_status, string $old_status, WP_Post $post ): void {
		...
		if ( ( 'draft' === $old_status || 'auto-draft' === $old_status ) && $new_status === 'publish' ) {
			...
			if ( $ai_content_generated && ! wp_is_post_revision( $post_id ) ) {
                ...
                $this->ai_content_published( $post_type, $post_id, 'ai_assistant_ui', [ 'correlation_id' => $correlation_id ] );
