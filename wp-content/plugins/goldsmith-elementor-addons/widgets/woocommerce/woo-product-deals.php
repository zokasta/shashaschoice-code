...
class Goldsmith_Woo_Product_Deals extends Widget_Base {
    use Goldsmith_Helper;
    public function get_name() {
        return 'goldsmith-woo-product-deals';
    }
    public function get_title() {
        return 'WC Product Deals (N)';
    }
    ...
    protected function register_controls() {
        ...
        $this->add_responsive_control( 'col',
            [
                ...
                'max' => 6,
                'default' => 2,
                'selectors' => ['{{WRAPPER}} .goldsmith-product-deals .row>.item-col' => '-ms-flex: 0 0 calc(100% / {{VALUE}} );flex: 0 0 calc(100% / {{VALUE}} );width: calc(100% / {{VALUE}} );'],
                'condition' => [ 'type' => 'grid' ]
            ]
        );
        $this->add_control('column_gap',
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .goldsmith-product-deals .row>.item-col' => 'padding: 0 {{SIZE}}px;margin-bottom: {{SIZE}}px;',
                    '{{WRAPPER}} .goldsmith-product-deals .row' => 'margin: 0 -{{SIZE}}px -{{SIZE}}px -{{SIZE}}px;'
                ],
                'condition' => [ 'type' => 'grid' ]
            ]
        );
        ...
        $this->add_responsive_control( 'spacing',
            [
                'label' => esc_html__( 'Bottom Spacing', 'goldsmith' ),
                'type' => Controls_Manager::SLIDER,
                'range' => [
                    'px' => [
                        'max' => 100
                ...
                'selectors' => [ '{{WRAPPER}} .deals-item' => 'margin-bottom: {{SIZE}}px;' ]
            ]
        );
        $this->end_controls_section();
        /*****   END CONTROLS SECTION   ******/
    ...
    protected function render() {
        ...
        if ( $the_query->have_posts() ) {
            echo '<div class="goldsmith-product-deals type-'.$settings['type'].'">';
                if ( $settings['title'] ) {
                    echo '<div class="deals-heading deals-heading-top">';
